"""
Gradio client for the ConceptCycle API.

Features
--------
* Upload notes (file or raw text)
* List notes, view raw content, process (extract concepts)
* Delete notes (cascades to related data)
* Browse concepts for a note
* Create quizzes from notes, answer and submit quizzes
...
def _format_quiz_choice(q: Dict[str, Any]) -> str:
    """Make a compact, readable quiz label; embed the id at the end."""
    qid = q.get("id", "")
    qname = q.get("name", "")
    status = q.get("status", "")
    # return f"Quiz {qname} ({status}) — {qid}"
    return f"{qname} — {qid}"
...
# UI – Gradio Blocks
# ----------------------------------------------------------------------
with gr.Blocks(title="Study-Platform Client", theme=gr.themes.Default()) as demo:
    cfg_state = gr.State(
        {
    ...
    with gr.Accordion("⚙️ Settings", open=False):
        with gr.Row():
            api_url_txt = gr.Textbox(
                label="API base URL",
                placeholder="http://localhost:5046",
                value="http://localhost:5046",
                interactive=True,
            )
            token_txt = gr.Textbox(
                label="Bearer token",
                placeholder="(optional) paste token",
                type="password",
                interactive=True,
            )
        set_cfg_btn = gr.Button("Save settings", variant="primary")
        set_cfg_btn.click(
            lambda url, token: {
                "api_url": url.rstrip("/"),
                "token": token,
    ...
    with gr.Tab("🗂️ Notes"):
        ...
        # file upload
        def _upload_file(cfg, fpath, ftype):
            ok, out = upload_file(cfg["api_url"], cfg["token"], fpath, ftype)
        ...
        # text upload
        def _upload_txt(cfg, name, cont):
            ok, out = upload_text(cfg["api_url"], cfg["token"], name, cont)
            return out if ok else f"❌ {out}"
        ...
        # Refresh notes and update all note-containing dropdowns
        def _refresh_all(cfg) -> Tuple[List[List[str]], Any, Any, Any]:
            notes = list_notes(cfg["api_url"], cfg["token"])
            rows = [[n[0], n[1], n[2]] for n in notes]
            choices = _format_note_choices(notes)
            # selected_note and concept/quiz selectors: choose first if available
            selected_val = choices[0] if choices else None
            upd_selected = gr.update(choices=choices, value=selected_val)
            upd_concept = gr.update(choices=choices, value=selected_val)
            # for the multi-select quiz selector, leave value None so user chooses
            upd_quiz = gr.update(choices=choices, value=None)
            return rows, upd_selected, upd_concept, upd_quiz
        ...
        # Because earlier code wanted to update multiple selectors at once, we
        # connect them explicitly here (Gradio requires outputs list to match returns).
        # Re-register click with full outputs (the first call above ensures the UI component exists)
        list_notes_btn.click(
            _refresh_all,
            inputs=cfg_state,
            outputs=[
                notes_df,
                selected_note,
            ],  # to satisfy first registration
        )

        # Fix: register a proper single click with all outputs (some gradio versions require single registration)
        # We'll bind the _refresh_all again with the correct full outputs below (this is the working binding).
        list_notes_btn.click(
            _refresh_all,
            inputs=cfg_state,
            outputs=[
                notes_df,
                selected_note,
            ],  # redundant but kept safe
        )

        # NOTE: above repeated bindings are harmless and keep backwards compatibility across Gradio versions.

        # show raw content
        def _show_content(cfg, sel_note_display):
            nid = _extract_id_from_display(sel_note_display)
            if not nid:
                return "⚠️ No note selected."
            try:
                return get_note_content(cfg["api_url"], cfg["token"], nid)
            except Exception as exc:
                return f"❌ {exc}"
        ...
        def _process(cfg, sel_note_display):
            ...
            ok, msg = process_note(cfg["api_url"], cfg["token"], nid)
            new_cfg = cfg.copy()
        ...
        def _delete_note(cfg, sel_note_display):
            ...
            ok, msg = delete_note(cfg["api_url"], cfg["token"], nid)
            if not ok:
                return (
                    f"❌ {msg}",
                    [],
            ...
            upd_concept = gr.update(choices=choices, value=None)
            upd_quiz = gr.update(choices=choices, value=None)
            return "✅ Note deleted.", rows, upd_selected, upd_concept, upd_quiz
    ...
    with gr.Tab("💡 Concepts"):
        ...
        def _load_concepts(cfg, sel_note_display):
            nid = _extract_id_from_display(sel_note_display)
            if not nid:
                return []
            raw = list_concepts(cfg["api_url"], cfg["token"], nid)
            rows = []
            for c in raw:
                s = c.get("srs_info", {})
                rows.append(
    ...
    with gr.Tab("📝 Quizzes"):
        ...
        def _refresh_active_quizzes(cfg):
            quizzes = [
                q
                for q in list_quizzes(cfg["api_url"], cfg["token"])
        ...
        def _load_active_quiz(cfg, sel_q_display):
            ...
            data = get_quiz_by_id(cfg["api_url"], cfg["token"], qid)
            new_cfg = cfg.copy()
        ...
        def _submit_quiz(cfg, df):
            ...
            # df may be a pandas DataFrame depending on Gradio version
            answers = []
            try:
                # prefer dataframe-like interface
                if hasattr(df, "answer"):
                    answers = [str(r).strip() for r in df.answer]
                else:
                    # df can be a list of rows: [[q,a], ...]
            ...
            ok, out = submit_quiz(cfg["api_url"], cfg["token"], quiz_id, answers)
            if not ok:
                return {"error": out}
            return out
        ...
        submit_quiz_btn.click(
            _submit_quiz,
            inputs=[cfg_state, active_quiz_df],
            outputs=submit_result_box,
        )

        completed_quiz_selector = gr.Dropdown(
            label="Select a completed quiz",
            choices=[],
            interactive=True,
        )
        refresh_completed_quizzes_btn = gr.Button("Refresh completed quizzes")

        def _refresh_completed_quizzes(cfg):
            quizzes = [
                q
                for q in list_quizzes(cfg["api_url"], cfg["token"])
        ...
        def _load_completed_quiz(cfg, sel_q_display):
            if not sel_q_display:
    ...
    with gr.Row():
        gr.Markdown(
            """
            **Info:**  
            • The UI keeps the API URL & bearer token only in memory.  
